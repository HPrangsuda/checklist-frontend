...
function MaintenanceEdit() {
  ...
  const existingUploaded = existingFiles.map(f => ({ id: f.fileName, name: f.fileName, size: f.fileSize, url: f.fileUrl, type: f.fileType }))
  ...
  const fetchData = async () => {
    try {
      ...
      if (data?.attachment) {
        try {
          const parsed = typeof data.attachment === 'string' ? JSON.parse(data.attachment) : data.attachment
  ...
  const handleDeleteFile = async (fileId: string) => {
    if (existingFiles.find(u => u.fileName === fileId)) {
      setExistingFiles(prev => prev.filter(u => u.fileName !== fileId)); return
    }
    const f = uploadedFiles.find(u => u.fileName === fileId)
    if (!f) return
    try {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const payload = {
        ...
        planDate:               formData.planDate               || null,
        startDate:              formData.startDate              || null,
        actualDate:             formData.actualDate             || null,
        status:                 formData.status                 || null,
        maintenanceBy:          formData.maintenanceBy          || null,
        responsibleMaintenance: formData.responsibleMaintenance || null,
      ...
      await api.put('/api/maintenance/update', fd)
  ...
  const handleChecklistSubmit = async () => {
    ...
    checklist.forEach(item => { if (!getAnswer(item).trim()) errs[`item_${item.id}`] = t('field_required') ?? 'Required' })
    if (Object.keys(errs).length) { setChecklistErrors(errs); toast.error(t('fill_required_fields')); return }
      ...
      <div className="px-2 pt-2 space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div className="space-y-2">
            ...
            <Input value={formData.responsibleMaintenance || ''}
              onChange={e => handleInputChange('responsibleMaintenance', e.target.value)}
          ...
          <CardContent className="p-6 space-y-4">
            <div className="space-y-1.5">
              ...
              <div className="relative">
                <select value={selectedStatus}
                  onChange={e => { setSelectedStatus(e.target.value); setChecklistErrors(p => { const n = {...p}; delete n.selectedStatus; return n }) }}
                  className={`w-full appearance-none border rounded-lg px-3 py-2.5 pr-9 bg-background text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 ${checklistErrors.selectedStatus ? 'border-red-400' : 'border-border'}`}>
                  <option value="">-- {t('please_select') ?? 'Select'} --</option>
                  {MACHINE_STATUSES.map(s => <option key={s} value={s}>{s}</option>)}
                </select>
                <ChevronDown className="absolute right-3 top-1/2 -translate-y-1/2 w-4 h-4 text-muted-foreground pointer-events-none" />
              </div>
              {checklistErrors.selectedStatus && <p className="text-red-500 text-xs flex items-center gap-1"><AlertCircle className="w-3 h-3" />{checklistErrors.selectedStatus}</p>}
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="space-y-1.5">
                <Label>{t('maintenance')}</Label>
                <div className="relative">
                  <select value={maintenanceBy} onChange={e => setMaintenanceBy(e.target.value as 'INTERNAL' | 'EXTERNAL')}
                    className="w-full appearance-none border rounded-lg px-3 py-2.5 pr-9 bg-background text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 border-border">
                    <option value="INTERNAL">INTERNAL</option>
                    <option value="EXTERNAL">EXTERNAL</option>
                  </select>
              ...
              {maintenanceBy === 'INTERNAL' && (
                <div className="space-y-1.5">
                  <Label>{t('responsible')}</Label>
                  <Input value={responsibleMaintenance2}
                    onChange={e => setResponsibleMaintenance2(e.target.value)}
                    placeholder={t('responsible')} />
                </div>
              )}
            </div>
                  ...
                  return (
                    <div key={item.id} className={`p-4 rounded-xl border ${hasError ? 'border-red-400 bg-red-50/10' : 'border-border bg-muted/20'}`}>
                      ...
                      {item.isChoice ? (
                        ...
                        <input type="text" value={answer} onChange={e => updateAnswer(item.id, e.target.value)}
                      ...
                      {hasError && <p className="text-red-500 text-xs mt-1 flex items-center gap-1"><AlertCircle className="w-3 h-3" />{t('field_required') ?? 'Required'}</p>}
                    </div>
                  )
                })}
              </div>
            ...
            <div className="flex justify-end pt-2">
              <Button type="button" onClick={handleChecklistSubmit} disabled={isSubmittingChecklist || !isChecklistValid()}>
        ...
        <Card>
          ...
          <CardContent className="p-6">
            <FileUploadField
              id="attachments" maxFiles={10}
              value={newFiles}
              uploadedFiles={existingUploaded}
