...
import { BillGenerate } from '@/module/account/bill/generate';

export const Route = createFileRoute('/account/billing-notes/')({
  component: DataTbl,
});
...
function DataTbl() {
  ...
  const onFetchData = async () => {
    try {
      ...
      const response = await api.get<PageResponse<DataDTO>>(`/api/account/billing-notes/get/page`, { params: queryParams });
      if (response.status) {
  ...
  const onDeleteData = async (): Promise<{ success: boolean }> => {
    ...
    try {
      const response = await api.delete<ResponseDTO<void>>(`/api/account/billing-notes/delete`, {
        headers: {
