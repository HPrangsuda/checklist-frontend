...
function DetailDrawer({ row, onClose }: { row: CalibrationDTO | null; onClose: () => void }) {
  ...
  return (
    <>
      ...
      <div ref={ref} role="dialog" aria-modal="true" aria-label={t('calibration_details')}
          ...
          <div className="flex-1 overflow-y-auto px-5 py-5 space-y-5">
            ...
            {row.results && (
              <div className="space-y-1">
                <p className="text-[11px] font-medium text-muted-foreground uppercase tracking-wide">{t('results')}</p>
                <p className={`text-sm font-semibold ${row.results === 'Pass' ? 'text-emerald-600' : 'text-red-600'}`}>{row.results}</p>
              </div>
            )}
          </div>
        )}
...
function CalibrationCard({ row, colConfig, selected, onClick, t }: {
  row: CalibrationDTO; colConfig: Omit<KanbanColumn, 'rows'>
...
function KanbanColumnView({ col, selectedUid, onSelect, t }: {
  ...
  onSelect: (row: CalibrationDTO) => void; t: (key: string) => string
}) {
  return (
    <div className="flex flex-col rounded-xl border flex-1 min-w-0 bg-muted/30">
      ...
      <div className="flex flex-col gap-2 p-2.5 overflow-y-auto" style={{ maxHeight: 480 }}>
        {col.rows.length === 0 ? (
          ...
          <CalibrationCard key={`${row.id}`} row={row} colConfig={col}
            selected={selectedUid === `${row.id}`} onClick={() => onSelect(row)} t={t} />
        ))}
      </div>
    </div>
  )
...
export function CalibrationKanbanCard({ year }: Props) {
  const { t }             = useTranslation('checklist')
  ...
  useEffect(() => { fetchData(debouncedKeyword, year) }, [debouncedKeyword, year])

  const fetchData = async (kw: string, yr: number) => {
    try {
      setLoading(true)
  ...
  const columns: KanbanColumn[] = useMemo(() => {
    ...
    const sortByDueAsc = (arr: CalibrationDTO[]) => [...arr].sort((a, b) => {
      const da = parseDate(a.dueDate), db = parseDate(b.dueDate)
      if (!da && !db) return 0; if (!da) return 1; if (!db) return -1
      return da.getTime() - db.getTime()
    })
    const sortByCertDesc = (arr: CalibrationDTO[]) => [...arr].sort((a, b) => {
    ...
    return COLUMN_CONFIG.map(cfg => ({
      ...cfg,
      rows: cfg.key === 'done' ? sortByCertDesc(buckets[cfg.key]) : sortByDueAsc(buckets[cfg.key]),
    }))
  }, [data])

  const selectedUid = selected ? `${selected.id}` : null
  const handleSelect = (row: CalibrationDTO) => { setSelected(prev => (prev?.id === row.id ? null : row)) }
