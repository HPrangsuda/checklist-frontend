...
function getColumnKey(row: MaintenanceDTO): ColumnKey {
  if (row.actualDate) return 'done'
...
function DetailDrawer({ row, onClose }: { row: MaintenanceDTO | null; onClose: () => void }) {
  ...
  const plan   = parseDate(row?.planDate)
              ...
              <div className="rounded-md bg-muted px-3 py-2.5 flex items-center gap-2">
                ...
                <span className="text-xs text-muted-foreground">
                  ...
                  <span className="font-medium text-foreground">
                    {Math.round((due.getTime() - plan.getTime()) / 86_400_000)} {t('plan_duration_days')}
                  </span>
                </span>
              </div>
            )}
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center justify-between gap-1 pt-0.5 flex-wrap">
        ...
        <span className="text-[10px] text-muted-foreground ml-auto">{t('round')} {row.round} · {fmtDate(due)}</span>
      </div>
    </button>
  )
}
