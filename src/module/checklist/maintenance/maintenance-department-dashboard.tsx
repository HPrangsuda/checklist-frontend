...
export function MaintenanceDepartmentDashboard() {
  ...
  const barColor  = (r: number) => r >= 80 ? 'bg-emerald-500' : r >= 60 ? 'bg-amber-400' : 'bg-red-700'
  ...
  return (
    <div className="space-y-4 mb-6">
        ...
        <CardHeader className="flex flex-row items-center justify-between flex-wrap gap-3 border-b px-5 py-4">
          <div>
            <CardTitle className="font-bold">
              <Wrench className="w-5 h-5 text-red-700 inline mr-2" />
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        {[
          { label: t('total_machines'),      value: String(totalMachines),          icon: <Wrench      className="w-7 h-7 text-muted-foreground/30" />, color: '' },
