...
function parseResponse(res: any): CalibrationStatsDTO[] {
...
export function CalibrationStats() {
  ...
  const [data, setData] = useState<CalibrationStatsDTO[]>([])
  ...
  useEffect(() => {
    const fetch = async () => {
      try {
        setLoading(true)
        const res = await api.get('/api/dashboard/get/calibration-stats')
    ...
    <Card className="shadow-sm border-dashboard-border">
      <CardHeader className="bg-dashboard-bg/50">
        <CardTitle className="font-semibold">
          {t('calibration_stats')} — {data[0]?.year || new Date().getFullYear()}
        </CardTitle>
      </CardHeader>
      <CardContent>
        {loading ? (
